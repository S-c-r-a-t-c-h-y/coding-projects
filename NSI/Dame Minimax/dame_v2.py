...
class Pion:
    ...
    def coups_jouables(self, board):
        ...
        if self.valeur in [1, 2]:  # gère les coups possibles pour les pions
            ...
            coups.extend(
                (board[self.j + offset][self.i - 1], board[self.j + offset][self.i + 1])
            )

            if (
                case1 := board[self.j + offset][self.i - 1]
            ).valeur in cibles and case1 in voisins:
            ...
            coups.extend(
                [voisin for voisin in self.voisins(board) if voisin.j < self.i]
            )
        elif self.valeur == 4:
            coups.extend(
                [voisin for voisin in self.voisins(board) if voisin.j > self.i]
...
board = [
    [
        Pion(1 if (i % 2 + j % 2) % 2 != 0 else 0, i, j)
        if j >= 6
        else Pion(2 if (i % 2 + j % 2) % 2 != 0 else 0, i, j)
        if j <= 3
        else Pion(0, i, j)
        for i in range(10)
    ]
