...
def eleves_prenom(conn, nom):
    cur = conn.cursor()
    cur.execute(f"SELECT Prenom FROM Eleve WHERE Nom='{nom}'")
...
def eleve_in(conn, nom, prenom):
    cur = conn.cursor()
    cur.execute(f"SELECT Nom, Prenom FROM Eleve WHERE Nom='{nom}' AND Prenom='{prenom}'")
...
def classe_eleve(conn, nom, prenom):
    cur = conn.cursor()
    cur.execute(
        f"SELECT Classe.nom FROM Classe JOIN Eleve ON Classe.IDClasse=Eleve.IDClasse WHERE Eleve.Nom='{nom}' AND Eleve.Prenom='{prenom}'"
    )
...
def cursus_eleve(conn, nom, prenom):
    ...
    cur.execute(
        f"SELECT DISTINCT Matiere FROM Cours JOIN Cursus ON Cours.IDCours = Cursus.IDCours JOIN Eleve ON Cursus.IDEleve = Eleve.IDEleve WHERE Eleve.Nom = '{nom}' AND Eleve.Prenom = '{prenom}'"
    )
    ...
    while continuer:
        ...
        if choix == 2:
            ...
            if eleve_in(conn, nom, prenom):
                ...
                print(f"{nom} {prenom} n'est pas dans l'établissement")
