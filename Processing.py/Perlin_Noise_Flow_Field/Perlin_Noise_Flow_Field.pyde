...
def fill_grid(noiseScale, off_x, off_y, z):
    ...
    for x in range(len(grid)):
        for y in range(len(grid[0])):
            grid[x][y] = PVector.fromAngle(radians(map(noise((x + off_x) * noiseScale, (y + off_y) * noiseScale, z * noiseScale), 0, 1, -360, 360)))
...
def draw_grid():
    ...
    for x in range(len(grid)):
        for y in range(len(grid[0])):
            line(x * scl, y * scl, x * scl + grid[x][y].x * scl, y * scl + grid[x][y].y * scl)
