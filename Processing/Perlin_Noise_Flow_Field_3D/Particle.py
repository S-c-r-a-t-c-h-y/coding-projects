class Particle:
    ...
    def __init__(self, x, y, maxspeed, scl, ps):
        self.pos = PVector(x, y, 0)
        self.vel = PVector.random3D()
        self.acc = PVector(0, 0, 0)
        self.maxspeed = maxspeed
        self.prevPos = PVector(x, y, 0)
        self.scl = scl
        self.ps = ps
    ...
    def display(self):
        ...
        line(self.pos.x, self.pos.y, self.pos.z, self.prevPos.x, self.prevPos.y, self.prevPos.z)
        self.updatePrev()
        
    def updatePrev(self):
        self.prevPos = PVector(self.pos.x, self.pos.y, self.pos.z)
    ...
    def edges(self):
        ...
        if self.pos.y < 0:
            self.pos.y = height - 1
            self.updatePrev()
        if self.pos.z > max(width, height) / 2:
            self.pos.z = -(max(width, height) / 2)
            self.updatePrev()
        if self.pos.z < -(max(width, height) / 2):
            self.pos.z = max(width, height) / 2
            self.updatePrev()
