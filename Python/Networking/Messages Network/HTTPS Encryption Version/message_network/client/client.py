...
class Client:
    def __init__(self, ui, host_address: str, host_port: int, name: str, password: str, xor_key: str, chat, debug: bool = False):
        self.debug = debug
        self.alive = True
        ...
        self.xor_key = bytes(xor_key, ENCODING, ENCODING_ERROR_TYPE)
    ...
    def encrypt_send(self, data: bytes, key=None):
        key = key or self.xor_key
        self.client.sendall(byte_xor(data, key))
    ...
    def establish_client_connection(self):
        try:
            ...
            rsa_public_key = self.client.recv(1024).decode(ENCODING)
            encoded_xor_key = encode_xor_key(self.xor_key, rsa_public_key).encode(ENCODING, ENCODING_ERROR_TYPE)

            self.client.send(encoded_xor_key)
    ...
    def message_reception_thread(self):
        ...
        while self.client.fileno() != -1:
            try:
                header, res = decapsulate(byte_xor(self.client.recv(10_000_000), self.xor_key))
    ...
    def handle_input(self, inp: str):
        ...
        try:
            ...
            self.encrypt_send(encapsulate(msg, header))
    ...
    def stop_stream(self):
        ...
        self.encrypt_send(encapsulate(b"", header))
