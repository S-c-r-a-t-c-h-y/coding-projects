...
class App:
    ...
    def join_server(self):
        ...
        xor_key = self.ui.join_encryption_key_input.text() or generate_random_xor_key()

        self.ui.chats[chat_name] = ChatTab(chat_name, self, self.ui)

        client = Client(self.ui, ip, port, name, password, xor_key, self.ui.chats[chat_name])
    ...
    def host_server(self):
        ...
        rsa_private = self.ui.host_private_key_input.text()
        rsa_public = self.ui.host_public_key_input.text()

        e, n1 = rsa_private.replace(" ", "").split(",")
        d, n2 = rsa_public.replace(" ", "").split(",")

        if n1 != n2:
            self.ui.show_popup("Invalid RSA keys", "You provided 2 different modulo values in the RSA keys.", icon=QMessageBox.Critical)
            return
        ...
        try:
            ...
            self.ui.show_popup("Invalid RSA keys", "Format for the RSA keys is invalid.", icon=QMessageBox.Critical)
            return

        if (((10000 ** d) % n) ** e) % n != 10000:
            self.ui.show_popup("Invalid RSA keys", "The RSA keys you provided is not a valid RSA key pair.", icon=QMessageBox.Critical)
            return

        self.hosted_server = Server(self.ui, ip, port, password, d, e, n, debug)
