...
while 1:
    ...
    if biggest_contour.size:
        img_warp = get_warp(img, biggest_contour)
        img_warp_gray = cv2.cvtColor(img_warp, cv2.COLOR_BGR2GRAY)
        img_warp_blur = cv2.medianBlur(img_warp_gray,5)
        img_processed = cv2.adaptiveThreshold(img_warp_blur ,255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        cv2.imshow("Warped Image", img_processed)
    else:
        img_warp = img.copy()
        img_processed = img.copy()
        img.copy()
    
    img_stack = stack_images(1, ([img, img_warp, img_processed]))
    cv2.imshow("WorkFlow", img_stack)
